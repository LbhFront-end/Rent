<template>
  <div class="main flex-x-center flex-y-center flex-col">
      <!-- 保洁评论 -->
      <div class="cleanComment flex-x-center flex-y-center flex-col" v-for="item in repairList" :key="item.id">
          <!-- 标题 -->
        ...
        <div class="cleanComments flex-left flex-y-center flex-col" v-if="sendComment(item.status)">
            ...
            <textarea name="" id="" cols="30" rows="8" placeholder="在此处输入评论内容" v-model="repairCommentsContent" :disabled="CommentDisable(item.status)"></textarea>       
            </div>
        </div> 
        <!-- 提交评论 -->
  ...
  created() {
    ...
    self.$axios
      ...
      .then(function(response) {
        self.repairList = response.data.data[0];
        self.comment = response.data.data[0].order.evaluate;
        // console.log(self.repairList);
      })
  ...
  methods: {
    ...
    sendComment(item) {
      if (item == 2) {
        ...
        self.repairCommentsContent = self.comment;
        return true;
      } else {
        return false;
      }
